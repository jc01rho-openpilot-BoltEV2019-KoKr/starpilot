  ...
  def update(self, CC, CS, now_nanos, starpilot_toggles):
    ...
    if self.frame % self.CCP.STEER_STEP == 0:
      if self.CP.flags & (VolkswagenFlags.MEB | VolkswagenFlags.MQB_EVO):
        ...
        if CC.latActive:
          hca_enabled = True
          steering_curvature = CS.curvature
          apply_curvature = actuators.curvature + (steering_curvature - CC.currentCurvature)
          apply_curvature = apply_std_curvature_limits(apply_curvature, self.apply_curvature_last, CS.out.vEgoRaw, steering_curvature,
      ...
      if self.CP.flags & VolkswagenFlags.STOCK_HCA_PRESENT and not self.CP.flags & (VolkswagenFlags.MEB | VolkswagenFlags.MQB_EVO):
    ...
    if self.frame % self.CCP.ACC_CONTROL_STEP == 0 and self.CP.openpilotLongitudinalControl and not CS.radar_disable_failed:
      ...
      if self.CP.flags & (VolkswagenFlags.MEB | VolkswagenFlags.MQB_EVO):
        ...
        lead_distance = 0.
        if False:
          self.long_jerk_control.update(CC.enabled, long_override, lead_distance, hud_control.leadVisible, accel, critical_state)
          self.long_limit_control.update(CC.enabled, CS.out.vEgoRaw, hud_control.setSpeed, lead_distance, hud_control.leadVisible, critical_state)
    ...
    if self.frame % self.CCP.ACC_HUD_STEP == 0 and self.CP.openpilotLongitudinalControl and not CS.radar_disable_failed:
      if self.CP.flags & (VolkswagenFlags.MEB | VolkswagenFlags.MQB_EVO):
        ...
        if CS.speed_limit != 0 and self.speed_limit_last != CS.speed_limit:
          self.speed_limit_changed_timer = self.frame
        self.speed_limit_last = CS.speed_limit
        sl_active = self.frame - self.speed_limit_changed_timer < 400
        speed_limit = CS.speed_limit_predicative if sl_predicative_active else (CS.speed_limit if sl_active else 0)
