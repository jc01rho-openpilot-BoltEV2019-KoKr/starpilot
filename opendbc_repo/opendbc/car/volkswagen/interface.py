...
class CarInterface(CarInterfaceBase):
  ...
  @staticmethod
  def pre_init(CP, can_recv, can_send):
  ...
  def _is_engine_state_allowed_meb(can_recv, timeout: float = 0.5) -> bool:
    ...
    # [Klemmen_Status_01][Klemmen_Status_Kl_15]
    ...
    while time.monotonic() < end_time:
      ...
      for packet in packets:
        for msg in packet:
          if msg.address != 0x3C0 or len(msg.dat) != 4:
            continue

          ignition_on = bool((msg.dat[2] >> 1) & 0x01)

          if ignition_on:
            carlog.warning(f"Engine state is not allowed: Klemmen_Status_Kl_15={ignition_on}")
            return False
          else:
            carlog.warning(f"Engine state is allowed: Klemmen_Status_Kl_15={ignition_on}")
            return True
