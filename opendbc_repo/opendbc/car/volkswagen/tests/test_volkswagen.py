    ...
    for platform in CAR:
      with subtests.test(platform=platform.value):
        ...
        for comp in CAR:
          ...
          distinct_model_years = (platform.config.model_years and comp.config.model_years and
                                  platform.config.model_years.isdisjoint(comp.config.model_years))
          assert not shared_chassis_codes or distinct_model_years, f"Shared chassis codes: {comp}"
      ...
      with subtests.test(platform=platform.name):
        for wmi in WMI:
          for chassis_code in platform.config.chassis_codes | {"00"}:
            for model_year in platform.config.model_years or {"0"}:
              vin = ["0"] * 17
              vin[0:3] = wmi
