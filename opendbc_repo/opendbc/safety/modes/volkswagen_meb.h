...
static const AngleSteeringLimits VOLKSWAGEN_MEB_STEERING_LIMITS = {
  ...
  .angle_rate_up_lookup = {.x = {0., 10., 35.}, .y = {0.00055, 0.00022, 0.00010}},
  .angle_rate_down_lookup = {.x = {0., 10., 35.}, .y = {0.00055, 0.00028, 0.00015}},
  .max_angle_error = 3000,
...
static const int VOLKSWAGEN_MEB_MAX_STEERING_POWER = 125;

static bool volkswagen_meb_steer_power_cmd_checks(int steer_power, bool steer_req) {
  bool violation = false;
  violation |= steer_req && safety_max_limit_check(steer_power, VOLKSWAGEN_MEB_MAX_STEERING_POWER, 0);
  violation |= !steer_req && (steer_power != 0);
  violation |= !(aol_allowed || controls_allowed) && steer_req;
  return violation;
}
...
static bool volkswagen_meb_tx_hook(const CANPacket_t *msg) {
  ...
  if (msg->addr == MSG_HCA_03) {
    ...
    if (volkswagen_meb_steer_power_cmd_checks(steer_power, steer_req)) {
      tx = false;
    }
  ...
  if (msg->addr == MSG_ACC_18) {
    ...
    bool accel_override_allowed = controls_allowed && gas_pressed_prev && (desired_accel == 0);
    if (longitudinal_accel_checks(desired_accel, VOLKSWAGEN_MEB_LONG_LIMITS) && !accel_override_allowed) {
      tx = false;
    }
  }
